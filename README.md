...
1. Open your terminal or IDE (e.g., VS Code).
2. **CRITICAL:** Ensure Microsoft Excel is completely closed. If the `Master_TestData.xlsx` file is open, it creates a hidden lock file that will crash the parser.
3. Ensure the target application is closed before starting the run (the script will handle launching and cleanup).
4. Execute the main driver script:
   ```bash
   node DriverScript/driver.js

5. Monitor the real-time execution logs in the terminal.

6. Upon completion, navigate to the newly generated Reports/Run_[Timestamp]/Consolidated/ folder and open the Execution_Dashboard_[Timestamp].html file in any browser to view the results.